...
public class ACHEntry {
	...
	public ACHEntry clone() {
		ACHEntry newAchEntry = new ACHEntry();
		newAchEntry.setEntryDetail(new ACHRecordEntryDetail(getEntryDetail()
				.toString()));
		Vector<ACHRecordAddenda> newAddendaRecs = new Vector<ACHRecordAddenda>(
				getAddendaRecs().size());
		for (int i = 0; i < getAddendaRecs().size(); i++) {
			newAddendaRecs.add(getAddendaRecs().get(i).clone());
		}
		newAchEntry.setAddendaRecs(newAddendaRecs);
		return newAchEntry;
	}
}
