...
public class ACHBatch {
	...
	public ACHBatch clone(){
		ACHBatch newAchBatch = new ACHBatch();
		newAchBatch.setBatchHeader(new ACHRecordBatchHeader(getBatchHeader().toString()));
		newAchBatch.setBatchControl(new ACHRecordBatchControl(getBatchControl().toString()));
		Vector<ACHEntry> newEntryRecs = new Vector<ACHEntry>(getEntryRecs().size());
		for (int i = 0; i < getEntryRecs().size(); i++){
			newEntryRecs.add(getEntryRecs().get(i).clone());
		}
		newAchBatch.setEntryRecs(newEntryRecs);
		return newAchBatch;
	}
}
